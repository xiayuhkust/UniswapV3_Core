...
abstract contract UniswapV3Pool is IUniswapV3Pool {
    ...
    mapping(int24 => Tick.Info) private ticks_;
    
    function ticks(int24 tick) external view override returns (TickInfo memory) {
        Tick.Info storage info = ticks_[tick];
        return TickInfo({
            ...
            secondsPerLiquidityOutsideX128: info.secondsPerLiquidityOutsideX128,
            secondsOutside: info.secondsOutside,
            secondsOutsideX32: 0
        });
    }
    ...
    function slot0() external view virtual returns (
        uint160 sqrtPriceX96,
        int24 tick,
        uint16 observationIndex,
        uint16 observationCardinality,
    ...
    function initialize(uint160 sqrtPriceX96) public virtual {
        if (slot0_.sqrtPriceX96 != 0) revert AlreadyInitialized();
        ...
        )
    {
        ...
        bool flippedLower = ticks_.update(
            params.lowerTick,
            slot0_.tick,
        ...
        bool flippedUpper = ticks_.update(
            params.upperTick,
            slot0_.tick,
            int128(params.liquidityDelta),
            feeGrowthGlobal0X128_,
        ...
        (uint256 feeGrowthInside0X128, uint256 feeGrowthInside1X128) = ticks_
            .getFeeGrowthInside(
                params.lowerTick,
                params.upperTick,
                slot0_.tick,
    ...
    function mint(
        ...
        require(lowerTick % int24(tickSpacing) == 0, "tick not spaced");
        require(upperTick % int24(tickSpacing) == 0, "tick not spaced");
        if (
            lowerTick >= upperTick ||
    ...
    function swap(
        ...
        bytes calldata data
    ) public virtual returns (int256 amount0, int256 amount1) {
        Slot0 memory _slot0 = slot0_;
        ...
        while (
            ...
            if (state.sqrtPriceX96 == sqrtPriceTargetX96) {
                break;
            }

            if (step.initialized) {
                int128 liquidityNet = ticks_.cross(
                    state.tick,
                    feeGrowthGlobal0X128,
                    feeGrowthGlobal1X128
                );
        ...
        // Handle token transfers and callbacks
        if (zeroForOne) {
